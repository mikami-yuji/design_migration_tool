...
python -m pip install watchdog>=3.0.0 pystray>=0.19.0 Pillow>=10.0.0 pypdfium2>=5.0.0 python -m PyInstaller --no-warn-script-location
if %ERRORLEVEL% neq 0 (
...
python -m PyInstaller --clean --onefile --windowed --noupx --name "%APP_NAME%" main.py
if %ERRORLEVEL% neq 0 (
